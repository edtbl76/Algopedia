...
This module provides a complete implementation of a min heap data structure,
...
class MinHeap:
    """
    A min heap implementation using a list-based binary heap.
    ...
    def add(self, value: int) -> None:
        """
        Add a new value to the heap while maintaining min heap property.
    ...
    def heapify_up(self) -> None:
        """
        Restore min heap property by bubbling up the last added element.

        Starting from the newly added element at the end, compare with parent
        and swap if current element is smaller. Continue until heap property
    ...
    def heapify_down(self) -> None:
        """
        Restore min heap property by bubbling down from the root.

        Starting from the root, compare with children and swap with the smaller child
        if the current node is larger. Continue until heap property is restored or
        ...
        while self._has_child(current_index):
            ...
            smallest_child_index = self._get_smallest_child_index(current_index)

            # get the current and smallest child values
            current = self.heap[current_index]
            smallest_child = self.heap[smallest_child_index]

            # swap if current element is larger than smallest child
    ...
    def remove_min(self) -> Optional[int]:
        ...
        The root element (minimum) is replaced with the last element,
        then heapify_down is called to restore the heap property.

        Returns:
            Optional[int]: The minimum element, or None if heap is empty
        ...
        min_value = self.heap[self._ROOT_INDEX]
    ...
    def _get_smallest_child_index(self, index: int) -> int:
        """
        Get the index of the smallest child of the given node.

        Compares left and right children (if they exist) and returns
        the index of the smaller one.

        Args:
            index (int): Index of the parent node

        Returns:
            int: Index of the smallest child
        ...
        # Both children exist, compare and return the smallest
